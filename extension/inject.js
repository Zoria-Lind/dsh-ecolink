...
;(function () {
  ...
  const SUBSTRING_BLACKLIST = ['brief fact', 'placeholder', 'example_name', 'extracted_name', 'snake_case_key', 'fact from user', '记忆内容', '输出记忆标签', 'key: fact', '开头,紧接着写事实,以', '开头,紧接着写该条内容,以', '三部分连在一起']
  ...
  function maybeInject(payload) {
    ...
    if (compressMode.active && !compressInjected && typeof payload.prompt === 'string') {
      compressInjected = true
      injectedSessions.add(sid)
      const composed = buildCompressBlock(compressMode.oldItems ?? []) + '\n\n' + stripInjectedBlock(payload.prompt)
    ...
    if (settings.singleInjection && injectedSessions.has(sid)) { dbg('本页已注入过该会话,跳过'); return { changed: false } }
    if (typeof payload.prompt !== 'string' || payload.prompt.trim().length === 0) return { changed: false }
    const clean = stripInjectedBlock(payload.prompt)
    ...
    return { changed: composed !== payload.prompt, payload: { ...payload, prompt: composed } }
  }
  ...
  window.fetch = async function (input, init) {
    try {
      ...
      if (isCompletion && settings.injectEnabled) {
        ...
        if (!bodyText) return origFetch.apply(this, arguments)
        let payload
        try { payload = JSON.parse(bodyText) } catch { return origFetch.apply(this, arguments) }
        const out = maybeInject(payload)
        if (!out.changed) return origFetch.apply(this, arguments)
        ...
        return origFetch.call(this, typeof input === 'string' || input instanceof URL ? input : input.url, init2)
      }
  ...
  function harvestAndStripNode(node) {
    try {
      ...
      const cleaned = text.replace(/<DSM:memory_write(?:[^>]*)>[\s\S]*?<\/DSM:memory_write>/gi, '').replace(BLOCK_RE, '')
      if (cleaned !== text) {
  ...
  try {
    const observer = new MutationObserver(() => walkDom())
    const startObserve = () => { try { observer.observe(document.body, { subtree: true, childList: true, characterData: true }) } catch { /* ignore */ } }
